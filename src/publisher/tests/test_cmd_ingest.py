...
class CLI(base.BaseCase):
    ...
    def test_ingest_publish_dry_run_from_cli(self):
        ...
        result = json.loads(stdout)
        self.assertTrue(
            utils.has_all_keys(result, ["status", "id", "datetime", "dry-run"])
        )
        # ensure response data is correct
    ...
    def test_ingest_identical_from_cli(self):
        ...
        models.ArticleVersion.objects.all().update(
            datetime_record_updated=dummy_dt
        )  # bypasses auto_now=True in field
