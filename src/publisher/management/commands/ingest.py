    ...
    struct = OrderedDict(
        [
            (
                "status",
                errtype,
            ),  # final status of request (ingested, published, validated, invalid, error)
            (
                "code",
                code,
            ),  # the error classification (bad request, unknown, parse error, etc)
            (
                "comment",
                codes.explain(code),
            ),  # a generic explanation about the error code
            ("message", message),  # an explanation of the actual error
            (
                "trace",
                None,
            ),  # optional tracing information. if validation error, it should be error and it's context
            ("dry-run", dry_run),
            ("force", force),
        ]
    )
...
def success(print_queue, action, av, force, dry_run, log_context):
    lu = {INGEST: INGESTED, PUBLISH: PUBLISHED, INGEST_PUBLISH: PUBLISHED}
    ...
    attr = (
        "datetime_published" if orig_status == PUBLISHED else "datetime_record_updated"
    )
    struct = OrderedDict(
        [
            ("status", status),
            (
                "id",
                log_context["msid"],
            ),  # good idea? #'id': None if dry_run else av.article.manuscript_id,
...
def handle_single(print_queue, action, infile, msid, version, force, dry_run):
    ...
    log_context = {"msid": msid, "version": version, "identical": False}

    LOG.info(
        "attempting to %s article %s v%s", action, msid, version, extra=log_context
    )
    ...
    try:
        if action not in [PUBLISH]:
            ...
            log_context["data"] = (
                str(raw_data[:25]) + "... (truncated)" if raw_data else ""
            )
    ...
    choices = {
        ...
        INGEST: lambda msid, ver, force, data, dry: ajson_ingestor.ingest(
            data, force, dry_run=dry
        ),
        PUBLISH: lambda msid, ver, force, data, dry: ajson_ingestor.publish(
            msid, ver, force, dry_run=dry
        ),
        INGEST_PUBLISH: lambda msid, ver, force, data, dry: ajson_ingestor.ingest_publish(
            data, force, dry_run=dry
        ),
    }
...
def handle_many_concurrently(print_queue, action, path, force, dry_run):
    try:
        ...
        Parallel(n_jobs=-1, timeout=10)(
            delayed(job)(print_queue, action, path, force, dry_run)
            for path in ajson_file_list
...
def handle_many_serially(print_queue, action, path, force, dry_run):
    try:
        ...
        aws_events.notify(STOP)  # defer sending notifications
        num_failed = sum(
            [job(print_queue, action, path, force, dry_run) for path in ajson_file_list]
        )
        clean_up(print_queue)
        sys.exit(num_failed)
    finally:
...
class Command(ModCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--ingest", dest="action", action="store_const", const=INGEST
        )
        parser.add_argument(
            "--publish", dest="action", action="store_const", const=PUBLISH
        )
        parser.add_argument(
            "--ingest+publish",
            dest="action",
            action="store_const",
            const=INGEST_PUBLISH,
        )
    ...
    def handle(self, *args, **options):
        ...
        self.log_context = {"action": action, "force?": force, "dry_run?": dry_run}

        if not action:
            self.invalid_args(
                "no action specified. I need either a 'ingest', 'publish' or 'ingest+publish' action"
            )
        ...
        if serial:
            ...
            if not os.environ.get("LAX_MULTIPROCESSING"):
                LOG.warning(
                    "we want to ingest articles in parallel but LAX_MULTIPROCESSING must be set. Args %s, options %s",
                    args,
                    options,
                )
        ...
        try:
            if path:
                if options["msid"] or options["version"]:
                    self.invalid_args(
                        "the 'id' and 'version' options are not required when a 'dir' option is passed"
                    )

                if not os.path.isdir(path):
                    self.invalid_args(
                        "the 'dir' option must point to a directory. got %r" % path
                    )
                ...
                if not (options["msid"] and options["version"]):
                    self.invalid_args(
                        "the 'id' and 'version' options are both required when a 'dir' option is not passed"
                    )
