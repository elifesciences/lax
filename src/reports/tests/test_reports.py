...
class TestReport(base.BaseCase):
    def setUp(self):
        ...
        for msid, ver, dtstr in cases:
            av = models.ArticleVersion.objects.get(
                article__manuscript_id=msid, version=ver
            )
    ...
    def test_paw_recent_report_data2(self):
        ...
        models.ArticleVersion.objects.filter(
            article__manuscript_id=6250, version=2
        ).update(status=models.VOR)
    ...
    def test_paw_recent_report_date_updated(self):
        ...
        models.ArticleVersion.objects.filter(
            article__manuscript_id=6250, version=2
        ).update(datetime_published=dummy1)
        models.ArticleVersion.objects.filter(
            article__manuscript_id=6250, version=3
        ).update(datetime_published=dummy2)
        ...
        item = root.find(
            "./channel/item[guid='https://dx.doi.org/10.7554/eLife.06250']"
        )
        cases = [
        ...
        for path, expected in cases:
            ...
            self.assertEqual(
                actual,
                expected,
                "expecting {0} for path {1} got {2}".format(expected, path, actual),
            )
