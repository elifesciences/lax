...
class IngestIdentical(BaseCase):
    ...
    def test_ingest_identical_force(self):
        ...
        self.assertEqual(1, models.ArticleVersion.objects.count())
        self.assertRaises(
            fragment_logic.Identical, ajson_ingestor.ingest, self.ajson, force=True
        )
        self.assertEqual(1, models.ArticleVersion.objects.count())
    ...
    def test_incoming_ajson_structure_preserved(self):
        ...
        af = models.ArticleFragment.objects.get(
            type=models.XML2JSON, article=av.article
        )
...
class Ingest(BaseCase):
    ...
    def test_article_ingest_for_published_articles_succeeds_if_forced(self):
        ...
        # attempt another ingest
        attrs = {"title": "foo", "published": utils.todt("2012-2-2")}
    ...
    def test_out_of_sequence_ingest_fails(self):
        ...
        self.assertRaises(
            StateError, partial(ajson_ingestor.ingest, force=True), self.ajson
        )  # v2 POA
        self.assertEqual(models.Article.objects.count(), 0)
        self.assertEqual(models.ArticleVersion.objects.count(), 0)
...
class Publish(BaseCase):
    ...
    @skip(
        "forced pubdate update of non-v1 articles disabled until xml supports version history"
    )
    ...
    def test_article_publish_v2_forced2(self):
        ...
        del self.ajson["article"][
            "versionDate"
        ]  # remember, this was copied from a v1 that had a versionDate!
...
class UnicodePreserved(BaseCase):
    ...
    def test_api_doesnt_mangle_unicode(self):
        ...
        resp = c.get(
            reverse(
                "v2:article-version",
                kwargs={"msid": self.msid, "version": self.version},
            )
        )
        data = json.loads(resp.content.decode("utf-8"))
        given = data["authors"][1]["name"]["preferred"]
