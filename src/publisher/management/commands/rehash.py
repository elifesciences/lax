...
class Command(BaseCommand):
    help = 'calculates the hash for the stored article-json. completes in less than 2 mins'
    ...
    def handle(self, *args, **options):
        ...
        try:
            with transaction.atomic():
                ...
                t.total = q.count()
                for av in q.iterator(): # .iterator() to use server-side cursor and avoid thrashing memory
