...
class RSSArticleFeedGenerator(Rss201rev2Feed):
    ...
    def add_item_elements(self, handler, item):
        ...
        if pubdate:
            ...
            LOG.warn(
                "no pubdate, skipping added a 'dc:date' element to rss feed", extra=item
            )
...
class SpecificArticleFeed(AbstractArticleFeed):
    def get_object(self, request, aid_list):
        ...
        return {"aid_list": aid_list, "doi_list": doi_list}

    def link(self, obj):
        return reverse(
            "rss-specific-article-list", kwargs={"aid_list": ",".join(obj["aid_list"])}
        )
...
class RecentArticleFeed(AbstractArticleFeed):
    ...
    def get_object(self, request, article_status, since):
        return {
            ...
            "original": {"article_status": article_status, "since": since},
...
urlpatterns = [
    url(
        r"^(?P<aid_list>(eLife\.\d{5}[,]?)+)$",
        SpecificArticleFeed(),
        name="rss-specific-article-list",
    ),
    url(
        r"^(?P<article_status>(poa\+vor)|(poa|vor))/last-(?P<since>\d{1,3})-days/$",
        RecentArticleFeed(),
        name="rss-recent-article-list",
    ),
]
